...
struct DateFormatEdgeCasesTests {
    ...
    func testDateFormattingLeapYear() throws {
        ...
        guard let leapDate = Calendar.current.date(from: components) else {
            Issue.record("Failed to create leap year date from components")
            return
        }
    ...
    func testDateFormattingYearBoundaries() throws {
        ...
        guard let yearEndDate = Calendar.current.date(from: components) else {
            Issue.record("Failed to create year-end date from components")
            return
        }
